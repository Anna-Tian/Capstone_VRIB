...
namespace SickscoreGames.HUDNavigationSystem
{
	public static class HUDNavigationExtensions
	{
		...
		{
			return screenPos.z > 0 && screenPos.x > 0 && screenPos.x < Camera.main.pixelWidth && screenPos.y > 0 && screenPos.y < Camera.main.pixelHeight;
		}
		...
		public static void SetMarkerPosition (this HUDNavigationElement element, NavigationElementType elementType, Vector3 markerPos, RectTransform parentRect = null)
		{
            ...
            {
                if (markerPos.x >= 0- 0.15*Camera.main.pixelWidth && markerPos.x <= Camera.main.pixelWidth && markerPos.z > 0)
                {
                    //element.CompassBar.transform.localPosition = new Vector3(markerPos.x - (0.7f * Camera.main.pixelWidth / 2), 0f, 0f) * 0.15f;
                    Vector3 objDir = element.transform.position - Camera.main.transform.position;
                    Vector3 CamDir = Camera.main.transform.forward;
                    objDir.y = 0;
                    CamDir.y = 0;
                    float angle = Vector3.SignedAngle(CamDir, objDir, Vector3.up);
                    float degreeToUIDegConst = 2f;
                    if (angle > 0)
                    {
                        //float backAngle = 180 - angle;
                        element.CompassBar.transform.localPosition = new Vector3(angle * degreeToUIDegConst, 0f, 0f);
                    }
                    else
                    {
                        //float backAngle = 180 + angle;
                ...
                else
                {
                    ...
                    {
                        float backAngle = 180 - angle;
                        element.CompassBar.transform.localPosition = new Vector3(-backAngle * degreeToUIDegConst, 0f, 0f);
                    }
                    else
                    {
                        float backAngle = 180 + angle;
                 ...
                 //begin offset from bar
                 if (element.CompassBar.transform.parent.parent.name.Contains("Behind"))
                {
                    element.CompassBar.transform.localPosition = new Vector3(element.CompassBar.transform.localPosition.x, 40f, 0f);
                }
                 else
                {
                    element.CompassBar.transform.localPosition = new Vector3(element.CompassBar.transform.localPosition.x, -30f, 0f);
                }
                 //end offset from bar
            }
            else if (elementType == NavigationElementType.Minimap)
                element.Minimap.transform.localPosition = markerPos;
            else if (elementType == NavigationElementType.ARC4)
                element.ARC4.transform.localPosition = markerPos;
		}
		...
		public static void SetMarkerActive (this HUDNavigationElement element, NavigationElementType elementType, bool value)
		{
			...
			switch (elementType) {
				...
				case NavigationElementType.Minimap:
					...
					break;
                case NavigationElementType.ARC4:
                    markerGO = element.ARC4.gameObject;
                    break;
				default:
					break;
			}
		...
		public static void SetIndicatorPosition (this HUDNavigationElement element, Vector3 indicatorPos, RectTransform parentRect)
		{
            ...
            element.Indicator.transform.localPosition = new Vector3 (indicatorPos.x + parentRect.localPosition.x, indicatorPos.y + parentRect.localPosition.y, 0f);
            element.Indicator.OffscreenDistanceText.transform.localPosition = -0.06f * element.Indicator.transform.localPosition;
           }
		...
		public static void SetIndicatorOffscreenRotation (this HUDNavigationElement element, Quaternion rotation)
		{
			...
			if (element.Indicator.OffscreenPointer != null)
				element.Indicator.OffscreenPointer.transform.localRotation = rotation;
		}
