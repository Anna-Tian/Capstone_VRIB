...
namespace SickscoreGames.HUDNavigationSystem
{
	...
	public class HUDNavigationSystem : MonoBehaviour
	{
        ...
        //ARC4 RADAR
        [Tooltip("Enable if you want to use the ARC4 Radar feature.")]
        public bool useARC4 = true;
        ...
        /// <summary>
		/// Enable / Disable the ARC4 feature in runtime.
		/// </summary>
		/// <param name="value">value</param>
		public void EnableARC4(bool value)
        {
            if (useARC4 != value)
            {
                useARC4 = value;
                _HUDNavigationCanvas.ShowARC4(value);
            }
        }
        #endregion
        ...
        void InitAllComponents ()
		{
            ...
            if (useARC4)
            {
                _HUDNavigationCanvas.InitARC4();
            }
            else _HUDNavigationCanvas.ShowARC4(false);
		}
		...
		void UpdateAllComponents ()
		{
            ...
            //Update ARC4
            if (useARC4)
                _HUDNavigationCanvas.UpdateARC4(rotationReference, RadarModes.RotateRadar);
		}
		...
		void UpdateNavigationElements ()
		{
			...
			foreach (HUDNavigationElement element in NavigationElements) {
				...
				if (!element.IsActive) {
                    ...
                    element.SetMarkerActive(NavigationElementType.ARC4, false);
                ...
                //VR Adjustments
                _screenPos.x -= 0.15f * PlayerCamera.pixelWidth;
                _screenPos.y -= 0.15f * PlayerCamera.pixelHeight;
                ...
                //Update ARC4
                if (useARC4 && element.ARC4 != null)
                    UpdateARC4Element(element, _screenPos, _distance);
			}
		}
		...
		void UpdateRadarElement (HUDNavigationElement element, Vector3 screenPos, float distance)
		{
            ...
            if (radarMode == RadarModes.RotateRadar) {
				if (element.rotateWithGameObject)
					element.Radar.transform.localRotation = Quaternion.Euler (new Vector3 (0f, 0f, -rotationReference.eulerAngles.y));
			} else {
				if (element.rotateWithGameObject)
					element.Radar.Icon.transform.localRotation = Quaternion.Euler (new Vector3 (0f, 0f, -element.transform.eulerAngles.y));
			}

			// keep marker icon identity rotation?
			if (!element.rotateWithGameObject)
				element.Radar.Icon.transform.localRotation = Quaternion.identity;
            ...
            element.Radar.DistanceText.text = Mathf.Round(distance) + "m";
        }
		#endregion
		...
		{			
            ...
            // set marker position
            if (screenPos.x >= 0-0.15*Camera.main.pixelWidth && screenPos.x <= Camera.main.pixelWidth && screenPos.z > 0)
            {
                element.CompassBar.transform.SetParent(_HUDNavigationCanvas.CompassBar.ElementContainer);
                element.SetMarkerPosition(NavigationElementType.CompassBar, screenPos, _HUDNavigationCanvas.CompassBar.ElementContainer);
            }
            else
            {
                element.CompassBar.transform.SetParent(_HUDNavigationCanvas.CompassBar.backElementContainer);
                element.SetMarkerPosition(NavigationElementType.CompassBar, screenPos, _HUDNavigationCanvas.CompassBar.backElementContainer);
            }
          }
		#endregion
		...
		void UpdateIndicatorElement (HUDNavigationElement element, Vector3 screenPos, float distance)
		{
			if (useIndicators && element.showIndicator) {
				...
				if ((distance > indicatorRadius && !element.ignoreIndicatorRadius)) {
                    ...
                    //_isElementOnScreen = false; //Always use offscreen indicators
					if (!_isElementOnScreen) {
						if (useOffscreenIndicators && element.showOffscreenIndicator) {
							...
							Vector3 screenCenter = new Vector3 (0.85f * PlayerCamera.pixelWidth, 0.85f * PlayerCamera.pixelHeight, 0f) / 2f;
							screenPos -= screenCenter;
							float angle = Mathf.Atan2 (screenPos.y, screenPos.x);
                            // Begin Top-Down Angle calc
                            Vector3 objDir = element.transform.position - Camera.main.transform.position;
                            Vector3 CamDir = Camera.main.transform.forward;
                            objDir.y = 0;
                            CamDir.y = 0;
                            angle = -Vector3.SignedAngle(CamDir, objDir, Vector3.up) * Mathf.Deg2Rad;
                            // End Top-Down angle calc
                    ...
                    Vector3 screenCenterVR = new Vector3(0.7f * PlayerCamera.pixelWidth,0.7f*PlayerCamera.pixelHeight, 0f) / 2f;
                    Vector3 screenCenterWeb = new Vector3(0.85f * PlayerCamera.pixelWidth, 0.85f * PlayerCamera.pixelHeight, 0f) / 2f;
                    //screenPos -= screenCenterVR;
                    screenPos -= screenCenterWeb; 
                    if (screenPos.y > Screen.height / 2) screenPos.y = Screen.height / 2;
                    //screenPos.y = 0;
                    ...
                    element.Indicator.OffscreenRect.localScale = new Vector3(2, 2, 1);
                    //element.Indicator.OnscreenRect.localScale = new Vector3(1.5f, 1.5f, 1);
                    element.Indicator.OnscreenRect.localScale = new Vector3(0, 0, 1); //Onscreen indicators invisible
        ...
        #region ARC4 Methods
        void UpdateARC4Element(HUDNavigationElement element, Vector3 screenPos, float distance)
        {
            // check if element is hidden within the ARC4
            //if (element.hideInRadar)
            //{
            //    element.SetMarkerActive(NavigationElementType.ARC4, false);
            //    return;
            ...
            // keep marker icon identity rotation?
            if (!element.rotateWithGameObject)
                element.ARC4.transform.localRotation = Quaternion.Euler(new Vector3(0f, 0f, rotationReference.eulerAngles.y));

            // set marker active
            element.SetMarkerActive(NavigationElementType.ARC4, true);

            // calculate marker position
            Vector3 posOffset = element.GetPositionOffset(PlayerController.position);
            float CameraRotation = -PlayerCamera.gameObject.transform.eulerAngles.y;
            posOffset = Quaternion.AngleAxis(CameraRotation, Vector3.up) * posOffset;
            Vector3 markerPos = new Vector3(posOffset.x, posOffset.z, 0f);
            markerPos.Normalize();

            //ellipse maths
            // line is y=tan(angle)x
            //elipse is (x/a)^2+(y/b)^2 = 1
            //solved equation is (x/a)^2+(tan(angle)x)^2 = 1
            float a = 726 / 2;
            float b = 155 / 2;
            //ellipse maths end

            Text dt = _HUDNavigationCanvas.ARC4.Degrees.transform.GetComponent<Text>();
            int bearing = (Mathf.RoundToInt(CameraRotation) * -1);
            dt.text = bearing.ToString();
            markerPos.x *= a;//0.85 to put it inside ring
            markerPos.y *= b;
            markerPos.y += 25;
            if (element.name.Contains("NorthHolder"))
            {
                markerPos.y -= 25;
                element.ARC4.transform.SetAsFirstSibling(); // if element is north arrow, put below all others
            }
            // set marker position
            element.SetMarkerPosition(NavigationElementType.ARC4, markerPos);
            // handle marker's above/below arrows
            element.ShowRadarAboveArrow(useRadarHeightSystem && element.useRadarHeightSystem && element.IsInRadarRadius && -posOffset.y < -radarDistanceAbove);
            element.ShowRadarBelowArrow(useRadarHeightSystem && element.useRadarHeightSystem && element.IsInRadarRadius && -posOffset.y > radarDistanceBelow);
            //handle text
            element.ARC4.DistanceText.text = Mathf.Round(distance) + "m";
            markerPos.x *= 0.15f; //0.1f
            markerPos.y *= 0.4f; //0.4f
            if (markerPos.y < 0) markerPos.y *= 2.5f;
            element.ARC4.DistanceText.transform.localPosition = markerPos;
        }
        #endregion
